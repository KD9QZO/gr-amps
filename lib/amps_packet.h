...
/*
 * FOCC frames are broken down into bursts; each burst is f broken down into segments. Segments are comprised of either
 * busy-idle bits (FOCC_BI_BIT) or message data (FOCC_MESSAGE; could be part of a word, dotting, or word sync). After
 * each burst is a FOCC_END.
 */
namespace gr {
namespace amps {

/*! \brief An enumeration of the burst states */
enum burst_state {
	FOCC_BI_BIT = 1,
	FOCC_MESSAGE = 2,
...
/*! \brief An enumeration of the FOCC streams */
enum focc_streams {
	STREAM_A = 1,
	STREAM_B = 2,
	STREAM_BOTH = 3
...
struct focc_segment {
	...
	focc_segment(burst_state nstate): state(nstate), length(0), encoded_data(NULL) {
	}

	focc_segment(char *ndata, int nlength, int samples_per_sym): state(FOCC_MESSAGE) {
...
public:
	...
	focc_frame(std::vector<focc_segment*> nsegs): segments(nsegs), is_ephemeral(false), is_filler(false) {
	}

	focc_frame(std::vector<focc_segment*> nsegs, bool ephemeral, bool filler): segments(nsegs), is_ephemeral(ephemeral), is_filler(filler) {
	}
...
public:
	...
	recc_word(const unsigned char *bytebuf) {
		F = ((bytebuf[0] & 1) == 1);
		NAWC = (((bytebuf[1] & 1) << 2) | ((bytebuf[2] & 1) << 1) | (bytebuf[3] & 1));
	}
};
...
/**
 * Decode numbers from a "Word of the Called-Address" word, according to 553 2.7.1.1. The digits table is Table 2.7.1-2.
 */
...
// Convert three MIN digits (ASCII characters) to a 10-bit binary value based on the algorithm in section 2.3.1.1-1.
